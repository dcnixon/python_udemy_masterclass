...
class DataListBox(Scrollbox):
    ...
    def on_select(self, event):
        if self.linked_box:
            ...
            # print("link_id query", self.sql_select + " WHERE " + self.field + "=?", value)
            # // SELECT title, _id FROM songs WHERE album=? ORDER BY track,title //
            # ? = 47 (should be 399 for Billy Idol greatest hits
            # // SELECT name, _id FROM albums WHERE name=? ('Greatest Hits',) //
            # // SELECT name, _id FROM artists WHERE name=? ('Billy Idol',) // not considered
