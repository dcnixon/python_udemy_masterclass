...
class DataListBox(Scrollbox):
    ...
    def __init__(self, window, connection, table, field, sort_order=None, **kwargs):
        super().__init__(window, **kwargs)
    ...
    def requery(self, link_value=None):
        print('link_value is this: ',link_value)
    ...
    def on_select(self, event):
        if self.linked_box:
            print("printing curselection: ", self.curselection())
            index = self.curselection()[0]
            value = self.get(index),

# TODO link_id  is returning the first ID that matches self.field but does not acount for duplicate album names from different artists
#             print("\n query for link_id is:\n", self.sql_select + " WHERE " + self.field + "=?", value, '\n')
            link_id = self.cursor.execute(self.sql_select + " WHERE " + self.field + "=?", value).fetchone()[1]
            # print("link id is: ", link_id)      # TODO delete this line
            #At this point
...
if __name__ == '__main__':
    ...
    # albumList.bind('<<ListboxSelect>>', on_select)
