...
for location in locations.keys():
    ...
    for key in exits.keys():
        if location in exits.get(key).values():
            exits_to_destination_1.append((key, locations.get(key)))
...
print("====================================")
for location in locations.keys():
    exits_to_destination_2 = [(key, locations[key]) for key in exits.keys() if location in exits[key].values()]
    print("locations leading to: {}".format(location), end="\t")
